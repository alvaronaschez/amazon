...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        if m := re.search(r"(\.0+)+$", version1): # noqa
            version1 = version1[:m.start()]
        if m := re.search(r"(\.0+)+$", version2): # noqa
            version2 = version2[:m.start()]
        version1 = tuple(int(x) for x in version1.split("."))
        version2 = tuple(int(x) for x in version2.split("."))
