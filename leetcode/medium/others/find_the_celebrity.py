...
not know person j. The celebrity is the person labeled as 1 because both 0 and
...
the contrary.
Remember that you won't have direct access to
...
class Solution:
    """
    O(n) because we are using a stack for candidates,
    if you use a queue instead the complexity increases to O(n²)
    in the worst case
    """
    ...
    def findCelebrity(self, n: int) -> int:
        ...
        people = set((range(n))) - {candidate}
        for a in people - {candidate} - known_by[candidate]:
            if not knows(a, candidate) or knows(candidate, a):
                return -1
        return candidate
